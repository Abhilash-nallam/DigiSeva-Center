...
export async function verifyTotp(secret: string, token: string, epochTolerance = 1): Promise<boolean> {
  const result = await verifyTotpToken({ secret, token, epochTolerance });
   return result.valid;
}
